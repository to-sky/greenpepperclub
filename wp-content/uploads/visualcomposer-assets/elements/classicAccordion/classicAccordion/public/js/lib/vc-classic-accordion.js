...
(function ($) {
  ...
  var Accordion, clickHandler, old, hashNavigation, setActiveSection;
  ...
  function Plugin (action, options) {
    ...
    return this.each(function () {
      ...
      $this = $(this);
      data = $this.data('vce.classic.accordion');
      if (!data) {
        data = new Accordion($this, $.extend(true, {}, options));
        $this.data('vce.classic.accordion', data);
      }
  ...
  Accordion.prototype.getTriggers = function () {
    var i;
    i = 0;
    var panels = this.getContainer().find('.vce-classic-accordion-panels')[0];

    return $(panels).find('> .vce-classic-accordion-panel > .vce-classic-accordion-section-inner > .vce-classic-accordion-panel-heading [data-vce-classic-accordion-element]').each(function () {
      var accordion, $this;
      $this = $(this);
      accordion = $this.data('vce.classic.accordion');
      if ('undefined' === typeof(accordion)) {
        $this.vceClassicAccordionElement();
        accordion = $this.data('vce.classic.accordion');
      }
  ...
  Accordion.prototype.show = function (opt) {
    ...
    $container.setAttribute('data-vcv-accordion-opened', $elementId);
    $container.dispatchEvent(event);

    that.triggerEvent('show.vce.classicAccordionElement', opt);
    ...
    if (that.isAnimated()) {
      that.triggerEvent('beforeShow.vce.classicAccordionElement');
      $target
        .queue(function (next) {
          $targetContent.one(Accordion.transitionName, function () {
            $target.removeAttr(that.animatingAttribute);
            $targetContent.attr('style', '');
            that.triggerEvent('afterShow.vce.classicAccordionElement', opt);
          });
  ...
  Accordion.prototype.hide = function (opt) {
    ...
    if (that.isAnimated()) {
      that.triggerEvent('beforeHide.vce.classicAccordionElement');
      $target
        .queue(function (next) {
          $targetContent.one(Accordion.transitionName, function () {
            $target.removeAttr(that.animatingAttribute);
            $targetContent.attr('style', '');
            that.triggerEvent('afterHide.vce.classicAccordionElement', opt);
          });
          Accordion.emulateTransitionEnd($targetContent, that.getAnimationDurationMilliseconds() + 100);
          next();
        })
        .queue(function (next) {
          ...
          that.triggerEvent('hide.vce.classicAccordionElement', opt);
          next();
        })
        .queue(function (next) {
          var height = $targetContent.height();
  ...
  Accordion.prototype.dropdown = function (opt) {
    ...
    if (this.isActive()) {
      ...
      Plugin.call($this, 'show', opt);
      $(document).on('click.vce.classicAccordionElement.data-api.dropdown', function (e) {
        Plugin.call($this, 'hide', opt);
        $(document).off(e);
      });
    }
  ...
  Accordion.prototype.showNext = function (opt) {
    ...
    if ($triggers.length) {
      if ($activeTriggers.length) {
        ...
        lastActiveAccordion = $activeTriggers.eq($activeTriggers.length - 1).vceClassicAccordionElement().data('vce.classic.accordion');
  ...
  old = $.fn.vceClassicAccordionElement;

  $.fn.vceClassicAccordionElement = Plugin;
  $.fn.vceClassicAccordionElement.Constructor = Accordion;

  // Accordion no conflict
  // ==========================
  $.fn.vceClassicAccordionElement.noConflict = function () {
    $.fn.vceClassicAccordionElement = old;
    return this;
  };
  ...
  hashNavigation = function () {
    ...
    $accordion = $targetElement.length && $targetElement.find('[data-vce-classic-accordion-element][href="' + hash + '"],[data-vce-classic-accordion-element][data-vce-target="' + hash + '"]');
    ...
    if ($accordion.length) {
      ...
      $accordion.closest(".vce-classic-accordion").attr("data-vcv-initialized", true).attr("data-vcv-hash-navigated", true)
    }
    setActiveSection('add');
  };

  setActiveSection = function (action, elementId) {
    var $accordion = $('.vce-classic-accordion:not([data-vcv-hash-navigated])');
    ...
    if (action === 'update') {
      ...
      if (el && el.hasClass('vce-global-element')) {
        $accordion = el.find('.vce-classic-accordion');
      } else {
        return;
      }
    }
    ...
    if (action && action === 'add' && elementId) {
      var id = '#el-' + elementId;
      $accordion = $(id + '.vce-classic-accordion');
    ...
    $accordion && $accordion.length && $accordion.each(function (index, element) {
      var $element = $(element);
      var activeSectionIndex = parseInt($element.attr('data-active-section'));
      var accordionPanelContainer = $element.find('.vce-classic-accordion-panels')[0];
      var accordionPanels = $(accordionPanelContainer).find('> .vce-classic-accordion-panel');
      var accordionInner = $element.find('.vce-classic-accordion-inner')[0];
      var activeId = null;
      var event = new CustomEvent('attrChange');

      activeSectionIndex = accordionPanels.length >= activeSectionIndex ? activeSectionIndex - 1 : 0;
      ...
      $(accordionPanels).each(function (i, elem) {
        ...
        if (i === activeSectionIndex) {
          ...
          $elem.find("> .vce-classic-accordion-section-inner > .vce-classic-accordion-panel-body").css("height", "");
          activeId = $elem.attr('data-model-id')
        }
      });

      accordionInner.setAttribute('data-vcv-accordion-opened', activeId);
      accordionInner.dispatchEvent(event);
    });
  };

  window.vcv ? window.vcv.on('ready', setActiveSection) : setActiveSection('add');

  $(window).on('hashchange.vce.classicAccordionElement', hashNavigation);
  $(document).on('click.vce.classicAccordionElement.data-api', '[data-vce-classic-accordion-element]', clickHandler);
  $(document).on('ready.vce.classicAccordionElement', hashNavigation);
  $(document).on('afterShow.vce.classicAccordionElement', function (e, opt) {
