...
(function ($) {
  ...
  Accordion.prototype.getTriggers = function () {
    var i;
    i = 0;
    var panels = this.getContainer().find('.vce-classic-tabs-panels')[0];

    return $(panels).find('> .vce-classic-tabs-panel > .vce-classic-tabs-tab-inner > .vce-classic-tabs-panel-heading [data-vce-classic-accordion]').each(function () {
      var accordion, $this;
      $this = $(this);
  ...
  Accordion.prototype.show = function (opt) {
    ...
    that.triggerEvent('show.vce.classicAccordion', opt);
    ...
    if (that.isAnimated()) {
      that.triggerEvent('beforeShow.vce.classicAccordion');
      $target
        .queue(function (next) {
          $targetContent.one(Accordion.transitionName, function () {
            $target.removeAttr(that.animatingAttribute);
            $targetContent.attr('style', '');
            that.triggerEvent('afterShow.vce.classicAccordion', opt);
          });
  ...
  Accordion.prototype.hide = function (opt) {
    ...
    if (that.isAnimated()) {
      that.triggerEvent('beforeHide.vce.classicAccordion');
      $target
        .queue(function (next) {
          $targetContent.one(Accordion.transitionName, function () {
            $target.removeAttr(that.animatingAttribute);
            $targetContent.attr('style', '');
            that.triggerEvent('afterHide.vce.classicAccordion', opt);
          });
          Accordion.emulateTransitionEnd($targetContent, that.getAnimationDurationMilliseconds() + 100);
          next();
        })
        .queue(function (next) {
          ...
          that.triggerEvent('hide.vce.classicAccordion', opt);
          next();
        })
        .queue(function (next) {
          var height = $targetContent.height();
  ...
  Accordion.prototype.dropdown = function (opt) {
    ...
    if (this.isActive()) {
      ...
      Plugin.call($this, 'show', opt);
      $(document).on('click.vce.classicAccordion.data-api.dropdown', function (e) {
        Plugin.call($this, 'hide', opt);
        $(document).off(e);
      });
    }
  ...
  Accordion.prototype.showNext = function (opt) {
    ...
    if ($triggers.length) {
      if ($activeTriggers.length) {
        ...
        lastActiveAccordion = $activeTriggers.eq($activeTriggers.length - 1).vceClassicAccordion().data('vce.classic.accordion');
  ...
  old = $.fn.vceClassicAccordion;

  $.fn.vceClassicAccordion = Plugin;
  $.fn.vceClassicAccordion.Constructor = Accordion;

  // Accordion no conflict
  // ==========================
  $.fn.vceClassicAccordion.noConflict = function () {
    $.fn.vceClassicAccordion = old;
    return this;
  };
  ...
  hashNavigation = function () {
    ...
    $accordion = $targetElement.length && $targetElement.find('[data-vce-classic-accordion][href="' + hash + '"],[data-vce-classic-accordion][data-vce-target="' + hash + '"]');
    ...
    if ($accordion.length) {
      ...
      $accordion.closest(".vce-classic-tabs").attr("data-vcv-initialized", true).attr("data-vcv-hash-navigated", true)
    }
    setActiveTab('add');
  };

  setActiveTab = function (action, elementId) {
    var $tabs = $('.vce-classic-tabs:not([data-vcv-hash-navigated])');
    ...
    $tabs && $tabs.each(function (index, element) {
      ...
      var accordionPanelContainer = $element.find('.vce-classic-tabs-panels')[0];
      var accordionPanels = $(accordionPanelContainer).find('> .vce-classic-tabs-panel');
      var tabsInner = $element.find('.vce-classic-tabs-inner')[0];
      var activeId = null;
      var event = new CustomEvent('attrChange');
      ...
      $(accordionPanels).each(function (i, elem) {
        ...
        if (i < activeTabIndex) {
          ...
          $elem.find("> .vce-classic-tabs-tab-inner > .vce-classic-tabs-panel-body").css("height", "");
  ...
  $(window).on('hashchange.vce.classicAccordion', hashNavigation);
  $(document).on('click.vce.classicAccordion.data-api', '[data-vce-classic-accordion]', clickHandler);
  $(document).on('ready.vce.classicAccordion', hashNavigation);
  $(document).on('afterShow.vce.classicAccordion', function (e, opt) {
